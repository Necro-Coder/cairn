...
**Tabs.** The active tab carries a 3px top edge and a tinted background, both in its module's colour. A temporary tab is in italics with a dotted underline. A title is truncated by CSS and trimmed at the source: a tab is a strip of window, and the thing written in it will one day be a name the person chose.
